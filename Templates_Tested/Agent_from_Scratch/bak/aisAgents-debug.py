...
#####################################################FIX################################################################################
# Define the Jinja2 template for the system prompt
system_prompt_template = """
You are a wise agent and run in a loop of Thought, Action, PAUSE, Observation.
...
class Agent:
    def __init__(self, llm: any, system_prompt: str = system_prompt_template, cot_example: str = "", tools: dict = {}, user_input: str = None) -> None:
        """
        Initialize the Agent with an LLM instance and an optional system message.
    ...
    def execute(self):
        """
        Execute the conversation by calling the LLM and returning the assistant's response.
    ...
    def run(self, max_iterations=10, query: str = "", tools=None, user_input=None):
        """
        Run a loop to interact with the agent, process tools, and handle the conversation.
            ...
            if "PAUSE" in result and "Action" in result:
                ...
                if action:
                    ...
                    if chosen_tool in tools:
                        ...
                        args_list = [arg.strip().strip("'\"") for arg in arg_str.split(",")]
                        print(args_list)

                        try:
                            result_tool = tools[chosen_tool](*args_list)
